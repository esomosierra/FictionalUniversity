...
function enqueue_unversity_files() {
    ...
    wp_enqueue_script( 'search', get_theme_file_uri('js/Search.js'), array('jquery'), microtime(), TRUE );
    wp_enqueue_script( 'mynotes', get_theme_file_uri('js/MyNotes.js'), array('jquery'), microtime(), TRUE );
    wp_enqueue_script( 'like', get_theme_file_uri('js/Like.js'), array('jquery'), microtime(), TRUE );
...
/* === 
    ...
    MODIFYING CUSTOM FIELDS IN REST API GETSON RESULTS DATA
    BY:
    ADDING CUSTOM FIELDS IN REST API GETJSON RESULTS DATA 
